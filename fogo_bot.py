...
# Bạn có thể hardcode, nhưng khuyến nghị dùng biến môi trường
...
def has_furbo_tweet(x_username: str, max_check: int = 25) -> bool:
    ...
    Uses snscrape (no X API required). Private/suspended accounts will return False.
    """
    if not SNSCRAPE_ENABLED:
        logger.error("snscrape not available. Install with: pip install snscrape")
        return False
    try:
        # Nếu account private/suspended -> .get_items() ném exception hoặc rỗng
...
async def send_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    2) If X not verified in last 24h -> show task board + OAuth link
    3) After connect (or already verified): check $FURBO tweet via snscrape
    4) Daily CAPTCHA
